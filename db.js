...
function transaction(fn) {
  // trả về một hàm cần await
  return async function() {
    const conn = await client.connect();
    try {
      await conn.query('BEGIN');
      // patch nhỏ: cung cấp “db tạm” sử dụng cùng connection
      const txApi = {
        get: async (s, ...a) => (await conn.query(toPgParams(s, a))).rows[0],
        all: async (s, ...a) => (await conn.query(toPgParams(s, a))).rows,
        run: async (s, ...a) => { await conn.query(toPgParams(s, a)); return { changes: 1 }; },
        prepare: (s)=>({
          get: (...a)=>txApi.get(s,...a),
          all: (...a)=>txApi.all(s,...a),
          run: (...a)=>txApi.run(s,...a),
...
function initSqlite() {
  ...
  // DDL giữ nguyên từ server.js
  sqliteDb.exec(`
    PRAGMA journal_mode = WAL;
    CREATE TABLE IF NOT EXISTS docs (
      id TEXT PRIMARY KEY, name TEXT, soHieu TEXT, loai TEXT, mucDo TEXT, donVi TEXT,
    ...
    CREATE TABLE IF NOT EXISTS users (
      ...
      userUnit TEXT, userChiBo TEXT, hash TEXT, active INTEGER DEFAULT 1,
...
function wrapSqlite() {
  return {
    ...
    transaction: (fn)=>() => { const tx = sqliteDb.transaction(()=>fn(module.exports)); tx(); }
  };
}
...
async function init() {
  if (isPg) { await initPg(); api = { get, all, run, exec, prepare, transaction }; }
  else { initSqlite(); api = wrapSqlite(); }
}
const ready = init();
